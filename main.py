...
# TODO: clean the code
# TODO: should I get a unique function to update the text, with flags to
# select between fumble and hit?
...
class ButtonSelector:
    ...
    Each method sets the text box in the interface with the right format for
    the required effect, be it a critical hit or a fumble.
    """

    def __init__(self, ui):
        # The interface from whence the buttons will be retrieved and where
...
"""
    ...
    def get_critical(self, crit_type: str):
        ...
        effects = [HITS.random_select(crit_type)
...
if __name__ == '__main__':
    ...
    # Interface
    ui_setter = Ui_MainWindow()

    # Main window
    mw = QMainWindow()

    # Setup UI
    ui_setter.setupUi(mw)

    selector = ButtonSelector(ui_setter)

    # Set signals and targets
    ui_setter.bludgeoning_btn.clicked.connect(
        lambda: selector.get_critical("bludgeoning"))
    ui_setter.slashing_btn.clicked.connect(
        lambda: selector.get_critical("slashing"))
    ui_setter.piercing_btn.clicked.connect(
        lambda: selector.get_critical("piercing"))
    ui_setter.magic_hit_btn.clicked.connect(
        lambda: selector.get_critical("magic"))

    ui_setter.melee_btn.clicked.connect(
        lambda: selector.get_fumble("melee"))
    ui_setter.ranged_btn.clicked.connect(
        lambda: selector.get_fumble("ranged"))
    ui_setter.natural_btn.clicked.connect(
        lambda: selector.get_fumble("natural"))
    ui_setter.magic_fumble_btn.clicked.connect(
        lambda: selector.get_fumble("magic"))
